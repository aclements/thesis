...
commutativity or pass them to \testgen (\S\ref{sec:tool:generator})
...
% %
% Python's imperative style is a natural fit for highly stateful
...
% for when operations commute.  For every set of operations of a
...
% which the operations commute.

This test codifies the definition of \SIM commutativity from
...
POSIX model we present in \S\ref{sec:model}, only a few types need special
handling beyond
...
model \code{rename}.  The first five lines declare symbolic
...
with \code{O_TRUNC}.]
% When analyzing interfaces by hand, many
...
case that \testgen generates is shown in Figure~\ref{fig:testgen}.
...
\caption{An example test case for two \code{rename} calls generated by
         \testgen for the model in Figure~\ref{fig:rename-spec}.}
\label{fig:testgen}
\end{figure}
...
for the implementation as well.  As we demonstrate in \S\ref{sec:model},
