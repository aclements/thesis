...
% \item Does non-commutativity limit the scalability of interfaces?
%   (\S\ref{sec:eval:non-commutativity})

% \item Does thinking about commutativity result in better scalability for
%   applications?  (\S\ref{sec:eval:app})
%   \XXX[AC][These are one question.]

% \item Does optimizing for commutativity/scalability harm performance?
%   This question we answer by looking at single-core performance and by
%   comparing with Linux. The results are woven into the evaluation
%   discussion of the above two questions.
...
In addition to the operations analyzed in \S\ref{sec:fs}, we scalably implemented
