...
%
This section introduces \refcache, a novel reference counting scheme
...
\refcache exploits a further weakening of this interface, targeted at
...
In \refcache, both \code{inc} and \code{dec} return nothing and hence
always commute, even if the count reaches zero.  A new, conceptually
global \code{review()} operation finds objects whose reference counts
...
%
\XXX[AC]{In the pseudo-code, \code{review} is called from each core,
  so it's not really a ``global'' operation.}
%
In effect, \refcache concentrates non-commutativity and the resulting
cache-line conflicts into \code{review}.  Even though \code{review} is
expensive and non-scalable, the \refcache implementation invokes it
...
\XXX[AC]{This could be read as \refcache is perfectly scalably
  commutative.  It isn't, but it makes sensible trade-offs.}

This separation of concerns between count manipulation and zero
detection allows \refcache to batch increments and decrements,
...
probability and \refcache itself induces only a small constant rate of
conflicts for global maintenance.

Furthermore, in contrast with most scalable
reference counting mechanisms (see \cref{sec:related}), \refcache
...
\XXX[AC]{Any other trade-offs from the RadixVM talk?}

\paragraph{Basic \refcache.}
...
\cref{fig:refcache-code}.  Each core maintains a hash table
