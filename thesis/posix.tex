...
\noindent
\cbstart The rule facilitates scalability reasoning at the interface and
specification level, and \SRI\ commutativity lets us apply the rule to
complex interfaces.
%\S\ref{sec:topic:strong-commutativity} gave a glimpse of this using
%\code{open}.
...
combined operation's commutativity.  \cbend
For example,
...
other properties.  \cbstart As a result, \code{fork} fails to commute
...
implementation.  \cbend
% As we show in \S\ref{sec:eval:app}, using \code{posix_spawn} instead of
...
\cbstart Many other POSIX interfaces get this right: \code{mmap} can
...
\paragraph{Permit weak ordering.} \cbend
