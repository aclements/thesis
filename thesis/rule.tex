...
\XXX[STATUS]{
  v3: Applied Eddie init-317-gdbaaa0d and Frans init-317-gdbaaa0d.
  Put properties of \SIM commutativity before definition.
...
%
We'll write invocations as left half-circles \inv{0}{A}
(``invoke A'') and responses as right half-circles
\res{0}{A} (``respond A''), where the letters match
invocations and their responses.
%
Color and vertical offset differentiate threads:
\inv{1}{A} and \inv{2}{B} are invocations on
different threads.
...
$H$ above is well-formed; checking this for the red thread $\thread{1}{$t$}$, we
see that the thread-restricted subhistory $\HRESTRICT{H}{\thread{1}{$t$}} =
\text{\scpline{\histrestrict{1}{\historyexample}}}$ formed
by selecting \(\thread{1}{$t$}\)'s actions from $H$ alternates invocations
...
%
If $H =
\text{\scpline{\inv{1}{A} \inv{2}{B} \res{1}{A} \inv{1}{C} \res{2}{B}
    \res{1}{C}}}$, then
%
$\text{\scpline{\inv{2}{B} \res{2}{B} \inv{1}{A} \res{1}{A} \inv{1}{C}
    \res{1}{C}}}$
is a reordering of $H$, but
$\text{\scpline{\inv{2}{B} \inv{1}{C} \res{2}{B} \res{1}{C}
    \inv{1}{A} \res{1}{A}}}$
...
\code{get}/\code{set} interface and the history
%
\[\HY =
  [\underbrace{
    \inv{1}{A}=\code{set(1)}, \res{1}{A},
    \inv{2}{B}=\code{set(2)}, \res{2}{B}}_{\HY_1},
   \underbrace{
     \inv{1}{C}=\code{set(2)}, \res{1}{C}}_{\HY_2}].
\]
%
\(\HY\) \SI-commutes because \code{set} returns
nothing and every reordering sets the underlying value to 2, so
...
Returning to the \code{get}/\code{set} example, while the sequence $Y$
...
\scpline{\inv{1}{A} \inv{2}{B} \res{2}{B} \res{1}{A}}:

\begin{CompactItemize}
\item $m(s_0, \text{\inv{1}{A}}) = \langle s_1, \thread{1}{\YIELD} \rangle$
\item $m(s_1, \text{\inv{2}{B}}) = \langle s_2, \thread{2}{\YIELD} \rangle$
\item $m(s_2, \thread{1}{\CONTINUE}) = \langle s_3, \thread{1}{\YIELD} \rangle$
\item $m(s_3, \thread{2}{\CONTINUE}) = \langle s_4, \text{\res{2}{B}}\rangle$
\item $m(s_4, \thread{1}{\CONTINUE}) = \langle s_5, \text{\res{1}{A}}\rangle$
\end{CompactItemize}
...
multicore hardware (\cref{sec:scalability}), so, given reasonable
...
\[H =
  [\underbrace{
    \inv{1}{A}=\code{iszero()}, \res{1}{A}=\code{false},
    \inv{2}{B}=\code{iszero()}, \res{2}{B}=\code{false}}_{\HAB},
   \underbrace{
    \inv{3}{C}=\code{dec()}, \res{3}{C}=1,
    \inv{1}{D}=\code{dec()}, \res{1}{D}=0}_{\HCD}]
\]
%
...
\[H' = [
   \rlap{$\overbrace{\phantom{
         ...
         \inv{2}{B}=\code{iszero()},
         \res{2}{B}=\code{false},
         \inv{3}{C}=\code{dec()}, \res{3}{C}}}^{\HABC'}$}
   \underbrace{
    \inv{1}{A}=\code{iszero()}, \res{1}{A}=\code{false},
    \inv{2}{B}=\code{iszero()}, \res{2}{B}=\code{false}}_{\HAB'},
   \underbrace{
    \inv{3}{C}=\code{dec()}, \res{3}{C},
    \inv{1}{D}=\code{dec()}, \res{1}{D}}_{\HCD'}]
\]
...
Each \code{dec} can modify its local counter,
...
\emph{not} conflict-free for $\HABC'$ because $\inv{3}{C}$
will write one component of the state that is read and summed by
$\inv{1}{A}$ (and $\inv{2}{B}$).  But, again,
...
~~~If \(\HEAD(s.h) = a\):
~~~    \(r \gets \CONTINUE\)
~~~else if \(a = \YIELD\) and \(\HEAD(s.h)\) is a response
...
\vspace{-\baselineskip}
\splitcaption{Constructed \emph{non-scalable} implementation $\NSIMP$ for
  history $H$}{and
...
%
As long as each invocation matches the next invocation in $H$,
$\NSIMP$ simply replays the
corresponding responses from $H$, without invoking the reference
implementation.
%
If the input invocations diverge from $H$, however, $\NSIMP$ can no
longer replay responses from $H$, so it enters \emph{emulation mode}.
%
This requires feeding $M$ all previously received invocations to
prepare its state.  After this, $\NSIMP$ responds by passing all
invocations to the reference implementation and returning its
responses.

A state $s$ for $\NSIMP$ contains two components.
%
First, $s.h$ either holds the portion of $H$ that remains to be replayed
or has the value \EMULATE, which denotes emulation mode.
$s.h$ is initialized to $H$.
%
Second, $s.\V{refstate}$ is the state of the reference implementation,
and starts as the value of the reference implementation's initial
state.
%
...
\rule{\columnwidth}{0.5pt}
\vspace{-\baselineskip}
\splitcaption{Constructed scalable implementation $m$ for history $H$}{and
  reference implementation $M$.}
...
\[H = [
   \rlap{$\overbrace{\phantom{
         \inv{1}{A}=\code{put(1)}, \res{1}{A},
         \inv{2}{B}=\code{put(1)}, \res{2}{B}}}^{\HAB}$}
   \inv{1}{A}=\code{put(1)}, \res{1}{A},
   \underbrace{
     \inv{2}{B}=\code{put(1)}, \res{2}{B},
     \inv{3}{C}=\code{max()}, \res{3}{C}=1}_{\HBC}].
\]
%
...
%Symmetric cases hold in the history $\scpline{\invres{2}{B} \invres{1}{A}
%  \invres{3}{C}}$.
