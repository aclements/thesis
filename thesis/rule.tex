\section{Commutativity to conflict-freedom}
\label{sec:rule}

\XXX[STATUS]{Expanded and reworked to focus on conflict-freedom vs
  SOSP.  May reorder with \cref{sec:scalability}.  Planning to add
...
What is the precise connection between interface commutativity and
conflict-free implementations?
%
\Thiscref{sec:rule} answers this question by developing and proving a
formalism based on abstract actions,
histories,
and implementations.
%
\Thiscref{sec:rule} also defines \SIM commutativity, a critical part of this
formalism that makes it possible to broadly apply the scalable
commutativity rule to complex software interfaces.
%
Our constructive proof of the formalism
...
\Thiscref{sec:rule} focuses on the connection between commutativity
and conflict-freedom and assumes conflict-freedom translates to
scalability on real hardware.  We verify this assumption and explore
its boundaries experimentally in the following
\lcnamecref{sec:scalability}.
...
%
This happens when the caller of an interface cannot distinguish which
order actions actually occurred in, either through those actions'
...
future \code{get} operations cannot distinguish reoderings of $\HY$.
%
...
% %
does not \SI-commute because some orders set the value to 1 and some
...
multicore hardware given reasonable workload assumptions, so we can
say that $m$ scales in the $\HY$ region of $H$.
...
A constructive proof for the scalable commutativity rule can be obtained by
building a conflict-free
...
\[H = [
   \rlap{$\overbrace{\phantom{
         ...
         \scpline{\scpi{B}{2}}=\code{put(1)}, \scpline{\scpr{B}{2}}}}^{H_1}$}
   \scpline{\scpi{A}{1}}=\code{put(1)}, \scpline{\scpr{A}{1}},
   \underbrace{
     \scpline{\scpi{B}{2}}=\code{put(1)}, \scpline{\scpr{B}{2}},
     \scpline{\scpi{C}{3}}=\code{max()}, \scpline{\scpr{C}{3}}=1}_{H_2}].
\]
%
Both $H_1$ and $H_2$ \SIM commute in $H$, but $H$ overall is not \SIM
commutative.
%
An implementation could store per-thread maxima reconciled by
\code{max} and be conflict-free for $H_1$.  Alternatively, it could use a
global maximum that \code{put} checked before writing.  This
is conflict-free for $H_2$.
...
Since $H_1$ and $H_2$ together span $H$, that means no single
implementation can be conflict-free for \emph{both} $H_1$ and $H_2$.
%
In the
...
%
Furthermore, as we show in \cref{sec:scalability}, some conflict-free
access patterns don't scale on real
machines.
%
We hope to investigate these problems in the future, but as we show below,
