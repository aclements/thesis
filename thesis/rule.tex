...
Consider a history $H = \HA \HCONCAT \HB$ (where $\HCONCAT$ concatenates
action sequences).
%
...
the same state'' after any $\HB'$.  It could mean that the internal,
...
discussed in \S\ref{intro}, conflict freedom is our proxy for
...
% together.]
%\XXX[AC][When does an implementation get invoked with \POLL?  Is this
...
\XXX[rtm][``the ... $\HB$ region'' seems to imply that $m$ must
...
that generates the same $\HB$ as $M$ and is conflict-free.'']

\XXX[E][In fact the construction does generate the same results as those
in $\HB$, for any ordering of the invocations of $\HB$. And this is true
even if the TRUE implementation, given a different order of invocations
than the original $\HB$ order, WOULD GENERATE DIFFERENT RESULTS. This is
important; it's what I'd like interface-based to mean.]

\XXX[AC][The construction works not just for $\HA \HCONCAT \HB$, but for any
$\HA \HCONCAT \HB'$ where $\HB'$ is a reordering of $\HB$.  Indeed this has to be
the case, since, by its very nature, the construction can't tell the
difference, but we write things as if it has to be $\HB$.  Should we be
clearer that the construction is conflict-free in any $\HB'$?]
...
any history $\HA\HCONCAT P$ where $P$ is a prefix of a
reordering of $\HB$, the constructed implementation's steps in $P$
are conflict-free. This means that, within the \SRI-commutative
region, $m$ scales.
...
~~~If \(\HEAD(s.h) = a\):
~~~    \(r \gets \POLL\)
~~~else if \(a = \POLL\) and \(\HEAD(s.h)\) is a response
...
receives \POLL\ invocations in a restricted way; these assumptions aren't
...
excludes responses; and adds \POLL\ invocations as appropriate.
...
%
The action order in a \SRI-commutative region doesn't matter by
definition.
%
...
~~~If \(\HEAD(s.h[t]) = a\):
~~~    \(r \gets \POLL\)
~~~else if \(a = \POLL\) and \(\HEAD(s.h[t])\) is a response
...
version of $M$ that scales over $\HB$ in $H = \HA\HCONCAT \HB$.
%
$m$ is similar to $\NSIMP$, but extends it with a \emph{conflict-free
  mode} used to execute actions in $\HB$.
%
