...
class AbstractTable(object):
    ...
    def filter(self, pred):
        """Filter this data source by calling pred on each row and
        yielding only those for which pred returns a true value."""
        return _Op(self.__schema, self.__op_filter, [self], pred)

    @staticmethod
    def __op_filter(children, pred):
